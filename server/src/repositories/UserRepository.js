...
export class PostgresUserRepository {
    ...
    async save(user) {
        ...
        if (user.getId()) {
            await this.db.query('UPDATE users SET name = $1, email = $2, mobile_number = $3, password_hash = $4 WHERE id = $5', [user.getName(), user.getEmail(), user.getMobileNumber(), passwordHash, user.getId()]);
            return user;
        }
        else {
            const result = await this.db.query('INSERT INTO users (name, email, mobile_number, password_hash) VALUES ($1, $2, $3, $4) RETURNING id', [user.getName(), user.getEmail(), user.getMobileNumber(), passwordHash]);
