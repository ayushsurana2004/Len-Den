...
export class SettlementService {
    ...
    public async initiateSettlement(payerId: number, payeeId: number, amount: number): Promise<{ id: number, key: string }> {
    ...
    public async confirmSettlement(settlementId: number, key: string): Promise<{ success: boolean; newKey: string }> {
        ...
        try {
            ...
            const groupKeyCheck = await client.query(
                `SELECT ug.settlement_key, ug.group_id FROM user_groups ug
                 WHERE ug.user_id = $1
                 AND ug.settlement_key = $2
                 AND ug.group_id IN (SELECT group_id FROM user_groups WHERE user_id = $3)
                 LIMIT 1`,
                [row.payee_id, key, row.payer_id]
            );
    ...
    public async getSimplifiedDebts(groupId: number): Promise<{ from: any, to: any, amount: number }[]> {
        ...
        members.forEach((m: any) => netBalances.set(m.id, 0));

        // Get all expense splits in this group
        const splitsRes = await this.db.query(
            `SELECT e.payer_id, es.user_id, es.amount
        ...
        splitsRes.rows.forEach((row: any) => {
        ...
        const settlementsRes = await this.db.query(
            `SELECT s.payer_id, s.payee_id, s.amount
             FROM settlements s
             WHERE s.status = 'SETTLED'
             AND (s.payer_id IN (SELECT user_id FROM user_groups WHERE group_id = $1)
                  AND s.payee_id IN (SELECT user_id FROM user_groups WHERE group_id = $1))`,
            [groupId]
        );

        settlementsRes.rows.forEach((row: any) => {
            const payerId = row.payer_id;
        ...
        const result: { from: any; to: any; amount: number }[] = [];
        let i = 0, j = 0;
